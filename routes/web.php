...
Route::middleware(['web', 'auth', 'impersonate'])->group(function () {
    ...
    // Route to handle OpenAI toggle
    Route::post('/chat/toggle-openai', [ChatController::class, 'toggleOpenAI'])->name('chat.toggleOpenAI');
    Route::post('/contacts/{contact}/clear-history', [ChatController::class, 'clearHistory'])->name('contacts.clear-history');

    // Route to handle sending message
    Route::post('/send-message', [ChatController::class, 'sendMessage'])->name('chat.sendMessage');

    //Route::post('/chat/toggle-openai', [ChatController::class, 'toggleOpenAI']);
    Route::post('/openai/generate', [OpenAiTestController::class, 'generateText']);
